...
export default class extends Component {
    constructor(props) {
        ...
        this.CMap = compose(
            ...
            <GoogleMap
                ...
                defaultOptions={{streetViewControl: false, zoomControl: false, fullscreenControl: false, mapTypeControl: false}}
            >
    ...
    componentDidMount() {
        document.querySelector('body').classList.add('without-padding');
    }

    componentWillUnmount() {
        document.querySelector('body').classList.remove('without-padding');
    }
    ...
    onOpen(driver) {
        ...
        drivers.forEach((item, i) => drivers[i].isOpen = false);
                ...
                <this.CMap center={{lat: 25.03, lng: 121.6}} driver={this.state.drivers[0]}>
                    ...
                    {this.state.drivers.map((driver) =>
                        driver.checks.map((check) =>
                            <Marker icon={check.status === 'waiting' ? markerCheckWaitingIconResized : check.status === 'success' ? markerCheckSuccessIconResized : markerCheckFailedIconResized}
                                    key={'check_' + check.id}
